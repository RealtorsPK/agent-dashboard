    ...
    <div className="flex flex-wrap gap-[16px]">
      {
        amenities.items.length ?
          amenities.items.map((item) => (
            <button className={`rounded-[5px] border-[1px] text-[12px] py-[6px] px-[15px] leading-[18px] ${selectedFeatures.includes(item.id) ? 'border-border-secondary text-text-tertiary bg-[#EFEAF7]' : 'text-text-primary border-border-primary bg-white'}  hover:bg-[#EFEAF7] hover:text-text-tertiary hover:border-border-secondary`} key={item.id} onClick={() => onChange(item.id)} type='button'>
              {item.name}
            </button>
          ))
          :
