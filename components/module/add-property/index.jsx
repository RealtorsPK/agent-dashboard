...
const InnerForm = ({ onSubmit, loading, formdata, setFormdata, socialLinks, setSocialLinks, selectedFeatures, setSelectedFeatures }) => {
  ...
  const cancel = () => {
    routes.push(`/dashboard/properties/${formdata.propertyType}/listing`);
  };
  ...
  return (
    <div>
      ...
      <div className="md:text-right px-[10px] md:p-[0px]">
        <button
            className="text-[14px] mr-[15px] border-[1px] border-border-primary py-[10.5px] px-[20px] rounded-[5px] bg-white hidden md:inline-block"
            onClick={() => cancel()}
            type="button"
        >
{'Cancel'}
        ...
        <button
            className="text-[14px] border-[1px] border-border-primary py-[10.5px] px-[20px] rounded-[5px] bg-background-primary text-white capitalize w-full md:w-fit"
            onClick={() => !loading && submit()}
            type="button"
        >
          {loading ? <Loader className="border-white" /> : `Submit ${propertyType} Property`}
        </button>
      </div>
    </div>
  );
