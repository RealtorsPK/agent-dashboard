    ...
    <div className="bg-white min-h-[calc(100vh-60px)] py-[20px]">
      {
        buttons.map((items) => (
          <div
              className={`${toggleDropdown === items.title ? '' : 'h-[41px]'} overflow-hidden transition ease-in-out duration-500`}
              key={items.title}
          >
            <button
                className={`block w-full text-left text-[14px] py-[10px] px-[20px] ${items.url === active ? 'text-[#000] font-medium' : 'text-[#888888]'}`}
              ...
              items.subMenu ?
                items.subMenu.map((subItems) => (
                  <div key={subItems.title}>
                    <button
                        className="block w-full text-left text-[14px] mb-[10px] px-[20px] ml-[15px] text-[#888888]"
